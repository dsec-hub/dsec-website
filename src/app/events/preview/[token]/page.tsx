...
import { getEventPreview } from "@/lib/api";
...
export default async function EventPreviewPage({ params }: Props) {
  ...
  const event = await getEventPreview(token);
  // Bad / expired token, or the event was archived/removed → 404, same as a
  ...
  return (
    <>
      ...
      {event.flagship ? <FlagshipEvent event={event} /> : <EventDetail event={event} />}
    </>
  );
}
    ...
    <div className="border-b-[3px] border-ink bg-yellow text-ink">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2.5 sm:px-6">
        ...
        <p className="font-mono text-xs font-bold sm:text-sm">
          Private preview of how this event will look once published.
        </p>
